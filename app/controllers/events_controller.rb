class EventsController < ApplicationController
  ...
  def index
    @events = Event.page(params[:page]).per(24)
    ...
    @events = by_date(@events, params[:start_date], params[:end_date]) if params[:start_date].present? || params[:end_date].present?

    @bg_img = '/assets/co.jpeg'
    @co_img = '/assets/berg.jpeg'
  end

  private

  def event_params
    params.require(:event).permit(:title, :web_source_id, :start_date, :end_date)
  end
